...
export default function DashboardPage() {
  ...
  // Simulate "AI processing" delay when generating report
  ...
  const activeAnomalies = anomaliData;
  ...
  return (
    <div className="flex min-h-screen bg-slate-950">
      ...
      {/* Main content */}
      <div className="flex-1 flex flex-col min-w-0 lg:ml-[72px] transition-all duration-300">
        <Topbar title="Dashboard Analitik" />
        ...
        <main className="flex-1 p-6 space-y-6 overflow-y-auto">
          ...
          {/* ---- Section: Self-Service Report ---- */}
          <section className="rounded-2xl border border-slate-700/50 bg-[#1e293b]/60 p-5 space-y-5">
            ...
            <div className="flex flex-wrap items-end gap-4">
              ...
              <div className="flex gap-2">
                ...
                <button
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium border border-slate-700 text-slate-400 hover:text-white hover:border-slate-600 hover:bg-slate-700/40 transition-all"
            ...
            {/* AI Insight */}
            {reportActive && !loadingReport && (
              <AIInsightCard insight={currentInsight} />
            )}
          </section>

          {/* ---- Bottom Grid: Top Produk + Anomali + Sales Perform ---- */}
              ...
              <div className="rounded-2xl border border-slate-700/50 bg-[#1e293b]/60 overflow-hidden">
                <div className="px-5 py-4 border-b border-slate-700/40 flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{activeAnomalies.length} isu aktif</p>
                  </div>
                </div>
                <div className="p-4 space-y-3">
                  {activeAnomalies.map((a) => (
                    <AnomalyFlag key={a.id} anomali={a} />
                  ))}
                </div>
              </div>
                        ...
                        <div className="h-1.5 rounded-full bg-slate-800">
                          <div
                            className={`h-full rounded-full transition-all duration-700 ${
                              isOver ? "bg-emerald-500" : isLow ? "bg-rose-500" : "bg-amber-500"
                            }`}
                            style={{ width: `${pct}%` }}
                          />
                        </div>
